@extends('layouts.app')
@section('title', __('messages.edit_chronicle'))

@section('content')
<div class="section">
    <div class="container" style="max-width: 700px;">
        <h1 class="section-title" style="margin-bottom: 2rem;">{{ __('messages.edit_chronicle') }}</h1>
        <div class="card card-elevated" style="padding: 2rem;">
            <form action="{{ route('author.chronicles.update', $chronicle) }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                <div style="margin-bottom: 1rem;">
                    <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">{{ __('messages.title') }} *</label>
                    <input type="text" name="title" value="{{ old('title', $chronicle->title) }}" required style="width: 100%; padding: 0.75rem; border: 2px solid #e2e8f0; border-radius: 12px;">
                </div>
                <div style="margin-bottom: 1rem;">
                    ...
                    <select name="book_id" style="width: 100%; padding: 0.75rem; border: 2px solid #e2e8f0; border-radius: 12px;">
                        ...
                        @foreach($books as $book)
                            <option value="{{ $book->id }}" {{ $chronicle->book_id == $book->id ? 'selected' : '' }}>{{ $book->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div style="margin-bottom: 1rem;">
                    <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">{{ __('messages.content') }} *</label>
                    <textarea name="content" rows="15" required style="width: 100%; padding: 0.75rem; border: 2px solid #e2e8f0; border-radius: 12px; resize: vertical;">{{ old('content', $chronicle->content) }}</textarea>
                </div>
